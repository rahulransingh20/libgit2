...
int git_stream_register(git_stream_registration *registration, int tls)
{
	git_stream_registration *target = tls ?
		&stream_registry.callbacks :
		&stream_registry.tls_callbacks;
	...
	if (registration && registration->version != GIT_STREAM_VERSION) {
		giterr_set(GITERR_NET, "invalid registration version");
		return -1;
	}
...
int git_stream_register_tls(git_stream_cb ctor)
{
	...
	if (ctor) {
		...
		return git_stream_register(&registration, 1);
	} else {
		return git_stream_register(NULL, 1);
	}
}
